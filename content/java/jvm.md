...
## 方法区（Method Area）
类(class)，函数function，常量final variable，实例变量member，静态变量static variable，实例变量member variable 
...
## JVM栈（JVM Stacks）
存变量以及返回值， LIFO(Last In First Out) 当函数执行完成以后会删除掉。是一个临时储存空间。
...
## PC Register Area
每个线程thread都有一个。是存JVM命令的地址
쓰레드마다 하나씩 생성. JVM 명령의 주소값이 저장되는 공간입니다.

## Native Method Stack Area

자바외다른 언어의 호출을 위해 할당되는 영역입니다. 
자바에서 C/C++의 메소드를 호출할 때 사용하는 Stack 영역이라고 생각하시면 됩니다.
